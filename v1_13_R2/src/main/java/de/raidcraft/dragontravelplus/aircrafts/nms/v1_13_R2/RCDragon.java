...
public class RCDragon extends EntityEnderDragon implements Aircraft<RCDragon> {
    ...
    @Override
    public void load(AircraftConfig config) {
        XperTick = config.getSpeedX();
        ZperTick = config.getSpeedZ();
        YperTick = config.getSpeedY();
        this.waypointRadius = config.getWaypointRadius();
        this.playerPitch = config.getPlayerPitch();
    }
